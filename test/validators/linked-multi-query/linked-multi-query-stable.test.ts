...
describe("Test linkedMultiQuery.circom", function () {
  ...
  const oneQuery = {
    ...
    circuitIds: [CircuitId.LinkedMultiQueryStable],
    groupID: 1,
    verifierID: 1,
  };
  ...
  async function deployContractsFixture() {
    ...
    ({ linkedMultiQueryValidatorStable: validator, groth16VerifierValidatorStub: groth16Verifier } =
  ...
  it("Check contract version", async function () {
    expect(await validator.VERSION()).to.equal(contractsInfo.VALIDATOR_LINKED_MULTI_QUERY_STABLE.version);
    expect(await validator.version()).to.equal(contractsInfo.VALIDATOR_LINKED_MULTI_QUERY_STABLE.version);
  });
