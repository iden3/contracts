...
import { LinkedMultiQueryStable5ValidatorWithGroth16VerifierStubModule } from "../../../ignition/modules/deployEverythingBasicStrategy/testHelpers";
...
describe("Test linkedMultiQuery.circom", function () {
  ...
  const proofForOneQuery = packZKProof(
    [linkId, merklized]
      .concat([operatorOutput1, "0", "0", "0", "0"])
      .concat([queryHash1, "0", "0", "0", "0"]),
    dummyZKProof[0],
    dummyZKProof[1],
    dummyZKProof[2],
  );
  ...
  const proofForTwoQueries = packZKProof(
    [linkId, merklized]
      .concat([operatorOutput1, operatorOutput2, "0", "0", "0"])
      .concat([queryHash1, queryHash2, "0", "0", "0"]),
    dummyZKProof[0],
    dummyZKProof[1],
    dummyZKProof[2],
  );
  ...
  async function deployContractsFixture() {
    ...
    ({
      linkedMultiQueryValidatorStable5: validator,
      groth16VerifierValidatorStub: groth16Verifier,
    } = await ignition.deploy(LinkedMultiQueryStable5ValidatorWithGroth16VerifierStubModule));
