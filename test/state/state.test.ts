...
describe("Check replacedAt timestamp expirations", () => {
  it("Should throw for non-existent state and GIST roots", async function () {
    ...
    await expect(state.getGistRootReplacedAt("0x0112", 10)).to.be.revertedWith(
      "GIST root entry not found",
    );

    await expect(state.getGistRootReplacedAt("0x0212", 10)).to.be.revertedWith(
    ...
    ).to.be.revertedWith("State entry not found");
