...
func TestSignVerifyPoseidonGoExpected(t *testing.T) {
	t.Skip("skipping test TestSignVerifyPoseidonGoExpected")
...
func TestSignVerifyPoseidon(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		prefix := new(big.Int).SetBytes(utils.SwapEndianness(s)).Bytes()
		...
		oldState := new(big.Int).SetBytes(utils.SwapEndianness(s))
		...
		newState := new(big.Int).SetBytes(utils.SwapEndianness(s))
