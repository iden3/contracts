...
const VCPaymentProxyModule = buildModule("VCPaymentProxyModule", (m) => {
  ...
  const VCPayment = m.contract("VCPayment");
  ...
  const proxy = m.contract("TransparentUpgradeableProxy", [VCPayment, proxyAdminOwner, "0x"]);
...
export const VCPaymentModule = buildModule("VCPaymentModule", (m) => {
  ...
  const VCPayment = m.contractAt("VCPayment", proxy);
