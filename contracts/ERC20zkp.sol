...
contract ERC20ZKP is ERC20, IERC20ZKP {
    ...
    function _mintWithProof(
        ...
        require(inputs[7] == query.schema, "wrong claim schema has been used for proof generation");
        require(inputs[8] == query.slotIndex, "wrong claim data slot has been for proof generation");
        require(inputs[9] == query.operator, "wrong query operator has been for proof generation");
        require(inputs[10] == query.value[0], "wrong comparison value has been for proof generation");


        require(circuitValidator.verify(inputs, a, b, c), "zero-knowledge proof is not valid");
